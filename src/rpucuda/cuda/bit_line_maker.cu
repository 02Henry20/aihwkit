...
// Shared-CURAND D pulse generation (sorting removed):
...
// - D is never sorted. Pulse positions remain exactly as generated by the shared CURAND sequence.
//
...
#define GET_COUNTS_INNER_LOOP(SCALEPROB, BATCH_IDX, SHARED_RNG_DELAY, USE_SHARED_RNG)            \
  /* STAGE 3.4 */                                                                                  \
  negative = value < (T)0.0;                                                                       \
  value = (negative) ? -value : value;                                                             \
                                                                                                   \
...
#define GET_COUNTS_LOOP(PROB, SIZE, COUNTS, SCALEPROB, SHARED_RNG_DELAY, USE_SHARED_RNG)        \
  sz = SIZE;                                                                                       \
  /* STAGE 3.3 */                                                                                  \
  if (sourceId < sz) {                                                                             \
    value = PROB[sourceId];                                                                        \
    c = &COUNTS[sourceId];                                                                         \
                                                                                                   \
    GET_COUNTS_INNER_LOOP(SCALEPROB, 0, SHARED_RNG_DELAY, USE_SHARED_RNG);                     \
  }

#define GET_COUNTS_LOOP_BATCH(                                                                     \
    PROB, SIZE, COUNTS, SCALEPROB, TRANS, OUTTRANS, SHARED_RNG_DELAY, USE_SHARED_RNG)           \
  {                                                                                                \
    sz = SIZE;                                                                                     \
    ...
    for (int i_stride = 0; i_stride < n32; i_stride += total_threads) {                           \
      ...
      if (sourceId < n) {                                                                          \
        ...
        GET_COUNTS_INNER_LOOP(                                                                     \
            SCALEPROB, batch_idx, SHARED_RNG_DELAY, USE_SHARED_RNG);                           \
      }                                                                                            \
    }                                                                                              \
  }
...
#define GET_COUNTS_SIMPLE_LOOP_BATCH(                                                              \
    ...
    SHARED_RNG_DELAY, USE_SHARED_RNG)                                                            \
  {                                                                                                \
    /* STAGE 3.3 BATCH */                                                                          \
...

{
  ...
  if (((tid < tid_nx) && (tid < x_size * m_batch)) ||
    ...
    GET_COUNTS_SIMPLE_LOOP_BATCH(
        ...
        tid_nd, RPU_BLM_SHARED_RNG_D_DELAY, true);
...
__global__ void kernelUpdateGetCounts_Loop2(
  ...
  if (tid < max_size) {
    ...
    // d input: one shared CURAND sequence across D features; preserve generated pulse positions
    GET_COUNTS_LOOP(
        d_prob, d_size, d_counts, d_scaleprob, RPU_BLM_SHARED_RNG_D_DELAY, true);

    // x input: original independent CURAND stream
    compute_noz_if = false;

    GET_COUNTS_LOOP(
        x_prob, x_size, x_counts, x_scaleprob, RPU_BLM_SHARED_RNG_X_DELAY, false);
