...
void BitLineMaker<T>::makeCounts(
  ...
  K_out_->resize(K_out_->size() + m_batch);
  ...
  if (use_umh) {
    ...
    if (um_if) {
      ...
      if (current_ublm_){
        ...
        thrust::copy(dev_ptr, dev_ptr + m_batch, K_out_->end() - m_batch);
      }else{
          thrust::fill(K_out_->end() - m_batch, K_out_->end(), static_cast<float>(current_BL_));
        }
      ...
      if ((m_batch == 1) && (possible_linear && (!simple_loop))) {
        ...
        if (!possible_linear) {
          ...
          kernelUpdateGetCounts_Loop2<<<nblocks, nthreads_, 0, s>>>(
              ...
              dev_d_counts_->getData(), dev_d_noz, Kplus1,
              context_->getRandomStates(nthreads_ * nblocks), res, sr);
