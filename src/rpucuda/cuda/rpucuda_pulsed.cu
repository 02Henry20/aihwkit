...
template <typename T> RPUCudaPulsed<T> &RPUCudaPulsed<T>::operator=(RPUCudaPulsed<T> &&other) {
  ...
  void RPUCudaPulsed<T>::populateParameter(
    ...
    if (p->up.pulse_type == PulseType::None) {
      ...
      SimpleRPUDeviceMetaParameter<T> dp_simple(
          *static_cast<SimpleRPUDeviceMetaParameter<T> *>(dp));
  ...
  template <typename T>
  void RPUCudaPulsed<T>::loadExtra(
  ...
  template <typename T> void RPUCudaPulsed<T>::setWeights(const T *host_source) {
    ...
    if (rpu_device_) {
      if (rpu_device_->onSetWeights(this->getWeightsPtr())) {
        ...
        rpucuda_device_->populateFrom(
            *rpu_device_); // device pars have changed (due to onSetWeights)
      }
    }
  ...
  void RPUCudaPulsed<T>::forwardIndexed(
      const T *X_input, T *D_output, int total_input_size, int m_batch, int dim3, bool trans,
      bool is_test) {
  ...
  void RPUCudaPulsed<T>::forwardIndexedSlice(
      const T *X_input, T *D_output, int total_input_size, int m_batch, int dim3, bool trans,
      int m_batch_slice, const int *batch_indices, bool is_test) {
  ...
  void RPUCudaPulsed<T>::forwardMatrixIterator(
      InputIteratorT X_input, OutputIteratorT D_output, int m_batch, bool x_trans, bool d_trans,
      bool is_test) {

    fb_pass_->forwardMatrixIterator(
        this->getFBWeightsCuda(is_test), X_input, this->getXSize(), x_trans, D_output,
        this->getDSize(), d_trans, m_batch, this->getFwdAlpha(), *f_iom_, getMetaPar().f_io,
        is_test);
  };
  ...
  void RPUCudaPulsed<T>::backwardIndexedSlice(
      const T *D_input, T *X_output, int total_output_size, int m_batch, int dim3, bool trans,
      int m_batch_slice, const int *batch_indices) {
  ...
  void RPUCudaPulsed<T>::backwardMatrixIterator(
    ...
    fb_pass_->backwardMatrixIterator(
        this->getFBWeightsCuda(false), D_input, this->getDSize(), d_trans, X_output,
        this->getXSize(), x_trans, m_batch, this->getBwdAlpha(), *b_iom_, getMetaPar().b_io);
  };
